...
try {
    switch ($action) {
        case 'package_save':
            $id = (int) ($_POST['id'] ?? 0);
            $optFloat = static function (string $key): ?float {
            ...
            $data = [
                ...
                ':tier_price_3_star' => $optFloat('tier_price_3_star'),
                ':tier_price_4_star' => $optFloat('tier_price_4_star'),
                ':tier_price_5_star' => $optFloat('tier_price_5_star'),
                ':ideal_for' => trim((string) ($_POST['ideal_for'] ?? '')),
                ':best_time_visit' => trim((string) ($_POST['best_time_visit'] ?? '')),
                ':covered_destinations' => trim((string) ($_POST['covered_destinations'] ?? '')),
                ':cancellation_policy' => trim((string) ($_POST['cancellation_policy'] ?? '')),
            ];

            if ($id > 0) {
                $data[':id'] = $id;
                $sql = 'UPDATE packages SET title=:title, category=:category, location=:location, price=:price, duration=:duration, short_desc=:short_desc, full_desc=:full_desc, highlights=:highlights, inclusions=:inclusions, exclusions=:exclusions, important_notes=:important_notes, terms=:terms, faq=:faq, blog_content=:blog_content, featured=:featured, is_featured=:is_featured, is_curated=:is_curated, is_trending=:is_trending, is_underrated=:is_underrated, status=:status, slug=:slug, meta_title=:meta_title, meta_description=:meta_description, tier_price_3_star=:tier_price_3_star, tier_price_4_star=:tier_price_4_star, tier_price_5_star=:tier_price_5_star, ideal_for=:ideal_for, best_time_visit=:best_time_visit, covered_destinations=:covered_destinations, cancellation_policy=:cancellation_policy, updated_at=NOW() WHERE id=:id';
                db()->prepare($sql)->execute($data);
                $packageId = $id;
                log_activity('package', "Updated package #{$id}");
            } else {
                $sql = 'INSERT INTO packages (title, category, location, price, duration, short_desc, full_desc, highlights, inclusions, exclusions, important_notes, terms, faq, blog_content, featured, is_featured, is_curated, is_trending, is_underrated, status, slug, meta_title, meta_description, tier_price_3_star, tier_price_4_star, tier_price_5_star, ideal_for, best_time_visit, covered_destinations, cancellation_policy, created_at, updated_at) VALUES (:title,:category,:location,:price,:duration,:short_desc,:full_desc,:highlights,:inclusions,:exclusions,:important_notes,:terms,:faq,:blog_content,:featured,:is_featured,:is_curated,:is_trending,:is_underrated,:status,:slug,:meta_title,:meta_description,:tier_price_3_star,:tier_price_4_star,:tier_price_5_star,:ideal_for,:best_time_visit,:covered_destinations,:cancellation_policy,NOW(),NOW())';
                db()->prepare($sql)->execute($data);
        ...
        case 'itinerary_save':
            ...
            $description = trim((string) ($_POST['description'] ?? ''));
            if ($packageId <= 0) {
                flash('error', 'Choose a package from the dropdown before saving.');
                redirect('/admin/itineraries.php');
            }
            if ($title === '' || $description === '') {
                flash('error', 'Day title and description are required.');
                redirect('/admin/itineraries.php');
            }
            $imagePath = null;
            ...
            if ($id > 0) {
                ...
                $dupStmt = db()->prepare('SELECT id FROM itineraries WHERE package_id=:p AND day_number=:d AND sort_order=:s LIMIT 1');
                $dupStmt->execute([
                    ':p' => $data[':package_id'],
                    ':d' => $data[':day_number'],
                    ':s' => $data[':sort_order'],
                ...
                if ($dupStmt->fetch()) {
                    flash('error', 'This package already has that day number (and sort order). Edit the existing block below instead of adding a duplicate.');
                    redirect('/admin/itineraries.php');
                }
                $data[':image_path'] = $imagePath;
        ...
        case 'itinerary_move':
            ...
            if ($id > 0 && ($dir === 'up' || $dir === 'down')) {
                $cur = db()->prepare('SELECT id, package_id, sort_order, day_number FROM itineraries WHERE id=:id LIMIT 1');
                $cur->execute([':id' => $id]);
                $row = $cur->fetch(PDO::FETCH_ASSOC);
                if ($row) {
                    $pid = (int) ($row['package_id'] ?? 0);
                    $list = db()->prepare('SELECT id, sort_order, day_number FROM itineraries WHERE package_id=:pid ORDER BY sort_order ASC, day_number ASC, id ASC');
                    $list->execute([':pid' => $pid]);
                    $ordered = $list->fetchAll(PDO::FETCH_ASSOC);
                    $idx = null;
                    ...
                    if ($idx !== null) {
                        ...
                        if ($j >= 0 && $j < count($ordered)) {
                            $a = $ordered[$idx];
                            $b = $ordered[$j];
                            $sa = (int) ($a['sort_order'] ?? 0);
                            $sb = (int) ($b['sort_order'] ?? 0);
                            db()->prepare('UPDATE itineraries SET sort_order=:s WHERE id=:id')->execute([':s' => $sb, ':id' => (int) $a['id']]);
                            db()->prepare('UPDATE itineraries SET sort_order=:s WHERE id=:id')->execute([':s' => $sa, ':id' => (int) $b['id']]);
                            log_activity('itinerary', "Moved itinerary #{$id} {$dir}");
                        }
                    }
                }
            }
        ...
        case 'itinerary_duplicate':
            $sourceId = (int) ($_POST['id'] ?? 0);
            if ($sourceId > 0) {
                $src = db()->prepare('SELECT * FROM itineraries WHERE id=:id LIMIT 1');
                $src->execute([':id' => $sourceId]);
                $row = $src->fetch(PDO::FETCH_ASSOC);
                if ($row) {
                    $packageId = (int) ($row['package_id'] ?? 0);
                    $maxDay = db()->prepare('SELECT MAX(day_number) AS m FROM itineraries WHERE package_id=:pid');
                    $maxDay->execute([':pid' => $packageId]);
                    $maxRow = $maxDay->fetch(PDO::FETCH_ASSOC);
                    $nextDay = (int) ($maxRow['m'] ?? 0) + 1;
                    $maxSort = db()->prepare('SELECT MAX(sort_order) AS m FROM itineraries WHERE package_id=:pid');
                    $maxSort->execute([':pid' => $packageId]);
                    $maxSortRow = $maxSort->fetch(PDO::FETCH_ASSOC);
                    $nextSort = (int) ($maxSortRow['m'] ?? 0) + 1;

                    db()->prepare('INSERT INTO itineraries (package_id, day_number, title, description, image_path, sort_order, created_at) VALUES (:package_id,:day_number,:title,:description,:image_path,:sort_order,NOW())')
                        ->execute([
                            ':package_id' => $packageId,
                            ':day_number' => $nextDay,
                    ...
                    $imgStmt = db()->prepare('SELECT image_path FROM itinerary_images WHERE itinerary_id=:id ORDER BY id ASC');
                    $imgStmt->execute([':id' => $sourceId]);
                    foreach ($imgStmt->fetchAll(PDO::FETCH_ASSOC) as $img) {
                        ...
                        db()->prepare('INSERT INTO itinerary_images (itinerary_id, image_path, created_at) VALUES (:itinerary_id,:path,NOW())')
                            ->execute([':itinerary_id' => $newId, ':path' => $path]);
                    }
                    log_activity('itinerary', "Duplicated itinerary #{$sourceId} to #{$newId}");
                    flash('success', 'Day duplicated. Edit titles and images as needed.');
                }
            }
            redirect('/admin/itineraries.php');
            break;
        ...
        case 'hotel_save':
            ...
            $starVal = ($starRating !== '' && is_numeric($starRating)) ? round((float) $starRating, 1) : null;
            if ($starVal !== null && ($starVal < 0 || $starVal > 5)) {
                $starVal = null;
            }
            ...
            if ($id > 0) {
                $row[':id'] = $id;
                $sql = 'UPDATE package_hotels SET package_id=:package_id,name=:name,location=:location,hotel_type=:hotel_type,notes=:notes,amenities=:amenities,description=:description,star_rating=:star_rating'
                ...
                $row[':image_path'] = $imagePath;
                db()->prepare('INSERT INTO package_hotels (package_id,name,location,hotel_type,image_path,notes,amenities,description,star_rating,created_at) VALUES (:package_id,:name,:location,:hotel_type,:image_path,:notes,:amenities,:description,:star_rating,NOW())')->execute($row);
                $id = (int) db()->lastInsertId();
            }
            if (!empty($_FILES['images']['name'][0])) {
                foreach ($_FILES['images']['name'] as $i => $unusedName) {
